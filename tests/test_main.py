...
def test_create_category(client):
    reg_res = client.post(
        "/auth/register",
        json={"username": "category_user", "password": "123"}
    )
...
def test_transaction_updates_balance(client):
    reg_res = client.post(
        "/auth/register",
        json={"username": "wallet_user", "password": "123"}
    )
    ...
    assert final_accs.status_code == 200
...
def test_expense_updates_category_total_and_account_balance(client):
    reg_res = client.post(
        "/auth/register",
        json={"username": "expense_total_user", "password": "123"}
    )
    token = reg_res.json()["access_token"]
    headers = {"Authorization": f"Bearer {token}"}
...
def test_categories_are_user_scoped_but_defaults_are_shared(client):
    first = client.post(
        "/auth/register",
        json={"username": "alice_cat", "password": "123"}
    )
    second = client.post(
        "/auth/register",
        json={"username": "bob_cat", "password": "123"}
    )
